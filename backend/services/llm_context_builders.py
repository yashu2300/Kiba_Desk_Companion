"""Build compact inputs used by Kiba's two LLM personas."""
...
def _event(
    ...
    return {
        ...
        "time_until_start": _duration(
            event.get(
                "seconds_until_start"
            )
        ),
        "time_until_end": _duration(
            event.get(
                "seconds_until_end"
            )
        ),
...
def build_conversation_payload(
    ...
    """Build context for a user-initiated conversation."""
    ...
    return {
        ...
        "current_context": {
            ...
            "person_at_desk": state.get(
                "person_at_desk"
            ),
            "person_is_owner": state.get(
                "owner_at_desk"
            ),
            "emotion_estimate": {
                "label": state.get(
...
def build_contextual_payload(
    ...
    if trigger_name == "calendar_event_soon":
        ...
        if owner_confirmed:
            meaning = (
                "The owner is visually confirmed "
                "at the desk, but no keyboard or "
                "mouse input was detected. They may "
                "be reading or thinking, so this is "
                "not proof of procrastination."
            )
        else:
            meaning = (
                "No keyboard or mouse input was "
                "detected. Camera presence is unknown, "
                "so the user may be away, reading, "
                "thinking, or taking a break."
            )
        ...
        payload["trigger_context"] = {
            ...
            "total_computer_input_inactive_time": (
                _duration(
                    metrics.get(
                        "total_qualified_inactive_s"
                    )
                )
            ),
            "recent_completed_inactivity_sessions": (
                _periods(
                    metrics.get(
                        "recent_inactivity_periods",
                        [],
                    )
                )
            ),
        ...
        payload["trigger_context"] = {
            ...
            "inactive_time_during_current_desk_period": (
                _duration(
                    metrics.get(
        ...
        payload["trigger_context"] = {
            "meaning": (
                ...
                "estimate, not a fact about the user's "
                "feelings."
            ),
            "owner_at_desk": state.get(
                "owner_at_desk"
